...
logging.basicConfig(filename=".heuristic2.log", level=logging.DEBUG)
...
class Lord:
    ...
    def reset_guess_real_intimacy(self):
        self.guessRealIntimacy = list(self.revealedIntimacy)

    def apply_guess_intimacy(self, player_id, value):
        self.guessRealIntimacy[player_id] += value

    def get_guess_intimacy_str(self):
        return " ".join(list(map(str, self.guessRealIntimacy)))
...
def policy(lords, realLove, negotiationCount, daylight):
    ...
    This heuristic is more aggressive because we will
    randomize other players 9 times, and find the best results
    each time.
    """
    command = []
...
def guess_players_move(lords, daylight, turn=0):
    ...
    """
    for lord in lords:
        lord.reset_guess_real_intimacy()
    ...
    if daylight == "D":
        for i in range(1, 4):
            cmd = [random.randint(0, 5), random.randint(0, 5), random.randint(0, 5), random.randint(0, 5),
                   random.randint(0, 5)]
            for lord_id in cmd:
